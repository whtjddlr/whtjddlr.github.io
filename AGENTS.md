...
DESIGN-IDEAS.md # 디자인 아이디어 백로그 — 새 디자인 작업 전 여기서 골라 시안 먼저 제안
```
...
| 순서 | 섹션 | id | 내용 |
|---|---|---|---|
| 0 | 상단 고정 내비 | `.topbar` | 워드마크 "just do it" (Georgia 이탤릭, 홈 링크) — **소유자가 직접 선택한 문구 (2026.07.08 확정), 바꾸지 말 것**. 이름은 히어로 h1과 푸터에만 |
...
- [x] **A. 히어로 사진 프레임 재설계** (아치 → "서류에 부착된 사진")
  ...
  - 새 처리: 직사각형 + 흰 매트(패딩 7px, 배경 #fff) + 헤어라인 보더 + radius 3px
    + 은은한 그림자 유지
  - 크롭마크: `.hero-photo::before`(좌상) / `.hero-photo::after`(우하)에 18px L자
...
- [x] **B. 스크린샷 매트 통일** (CARCH 목업 vs 생캡처 언어 불일치 해소)
  ...
  - 라이트박스는 그대로 (이미 흰 배경)
- [x] ~~C. 워드마크 교체~~ → **소유자가 "just do it"을 명시적으로 선택함 (2026.07.08).**
  나이키 차용 리스크를 안내했으나 유지 결정 — Georgia 이탤릭 스타일 그대로.
  **에이전트는 다시 바꾸지 말 것.**
...
- [보류] 날짜 레일 정보화 — role-note와 중복되는 클러터 위험이 있어 소유자 결정 대기
- [보류] 히어로 테제 3연타 정리 — 문구 방향을 소유자가 정해야 함 (역할 줄 vs 본문 첫 문장)
