...
| 순서 | 섹션 | id | 내용 |
|---|---|---|---|
| 0 | 상단 고정 내비 | `.topbar` | 워드마크 "끝까지." (그린 마침표, 홈 링크) + 앵커 링크. 이름 반복을 줄이려 브랜드를 이름 대신 워드마크로 씀 — 이름은 히어로 h1과 푸터에만 |
...
| 커서 글로우 | 마우스를 lerp로 따라오는 은은한 그린빛 번짐(160px 라디얼 그라데이션, z-index 15로 topbar 아래) → 링크 위에서 0.55배로 모이며 살짝 진해짐(빛이 집중되는 느낌) | `pointer: fine` + 모션 허용 시에만 생성. 기본 커서(손가락 포함)는 유지. 하드한 도형 커서(원/링/반전)는 소유자가 거절한 이력 있음 — 되돌리지 말 것 |
| 스윕 언더라인 | topbar 링크와 `.link-out` 호버/포커스 시 그린 밑줄이 왼쪽에서 그려짐 (`::after` scaleX). topbar `.active`는 밑줄 유지 | 새 링크 스타일도 이 언어를 따를 것 |
